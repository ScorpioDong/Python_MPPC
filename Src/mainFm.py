...
class MainFm(QtWidgets.QWidget):
    ...
    def connect_init(self):
        self.StartDlg.ui.start_Btn.clicked.connect(self.startdlg_start_clicked_slot)
        self.ParmasDlg.ui.startBtn.clicked.connect(self.parmasdlg_start_clicked_slot)
    ...
    def mppc_config(self):
        ...
        if result != mppcum1a.MPPC_SUCCESS:
            ...
            MsgBox.exec_()
        result = mppcum1a.MPPC_SetProperty(self.DeviceHandle, self.Parmas['gatetime']*1000, self.Parmas['datasize'])
        if result != mppcum1a.MPPC_SUCCESS:
            MsgBox = QtWidgets.QMessageBox()
            MsgBox.setWindowTitle("！")
        ...
        if self.Parmas['threshold_index'] <= mppcum1a.VTH_75:
            result = mppcum1a.MPPC_SetThreshold(self.DeviceHandle, self.Parmas['threshold_index'])
        else:
            result = mppcum1a.MPPC_SetThreshold(self.DeviceHandle, self.mppcum1a.VTH_OVER)
        if result != mppcum1a.MPPC_SUCCESS:
            MsgBox = QtWidgets.QMessageBox()
            MsgBox.setWindowTitle("！")
        ...
        if result == mppcum1a.MPPC_SUCCESS:
            if flag == mppcum1a.ENABLE:
                mppcum1a.MPPC_SetPeltierControl(self.DeviceHandle, self.mppcum1a.DISABLE)
        else:
            MsgBox = QtWidgets.QMessageBox()
            MsgBox.setWindowTitle("！")
        ...
        while True:
            mppcum1a.MPPC_SetPeltierControl(self.DeviceHandle, self.mppcum1a.ENABLE)
    ...
    def parmasdlg_start_clicked_slot(self):
        ...
        # else:
            ...
            self.Parmas['x_step'] = round(self.ParmasDlg.Parmas['pic_x_len']*400.0/(self.ParmasDlg.Parmas['pic_x_count'] - 1.0))
            self.Parmas['y_step'] = round(self.ParmasDlg.Parmas['pic_y_len']*400.0/(self.ParmasDlg.Parmas['pic_y_count'] - 1.0))
            self.Parmas['x_speed'] = self.ParmasDlg.Parmas['mt_x_speed']
            self.Parmas['y_speed'] = self.ParmasDlg.Parmas['mt_y_speed']
            self.Parmas['gatetime'] = int(self.ParmasDlg.ui.gatetime_Cmb.currentText())
            self.Parmas['datasize'] = round(self._CYCLE_TIME/self.Parmas['gatetime'])
